    ...
    <div className='w-full bg-white border border-light-100 rounded-lg p-5'>
      {formattedData && formattedData.map((item, index) => (
        <div key={index} className='mb-5'>
          <p className='text-primary-main text-xl font-semibold mb-2'>{item.admission.title}</p>

          {/* <ul className='list-disc list-inside text-gray-700'>
            {item.admissionCriteria &&  item.admissionCriteria!.map((criteria: AdmissionCriteriaType, i: number) => (
